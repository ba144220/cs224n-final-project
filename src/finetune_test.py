...
config.prompt_tuning_range = (4, 14)

bnb_config = BitsAndBytesConfig(
    # load_in_8bit=True,
)

model = LlamaPromptTuningLM.from_pretrained(MODEL_NAME, config=config, device_map="auto")
...
# Process the dataset
train_dataset = dataset['train'].map(format_boolq_to_dialog).shuffle(seed=42).select(range(2000))
